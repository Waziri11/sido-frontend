...
import { CalendarDays, Crosshair, List, Map as MapIcon, MapPin, MoreHorizontal, PauseCircle, Pencil, PlayCircle, Plus, Search, Trash2 } from 'lucide-react'
...
const lightTileUrl = import.meta.env.VITE_MAP_TILE_URL || defaultTileUrl
const darkTileUrl = import.meta.env.VITE_MAP_DARK_TILE_URL || lightTileUrl
const rasterStyle = dark => ({ version: 8, sources: { osm: { type: 'raster', tiles: [dark ? darkTileUrl : lightTileUrl], tileSize: 256, minzoom: 0, maxzoom: 19, attribution: '<a href="https://www.openstreetmap.org/copyright">© OpenStreetMap contributors</a>' } }, layers: [{ id: 'background', type: 'background', paint: { 'background-color': dark ? '#111817' : '#e6e2d9' } }, { id: 'osm', type: 'raster', source: 'osm' }] })
const mapStyle = dark => (dark ? import.meta.env.VITE_MAP_DARK_STYLE_URL : import.meta.env.VITE_MAP_STYLE_URL) || rasterStyle(dark)
const useFilteredDarkTiles = dark => dark && !import.meta.env.VITE_MAP_DARK_STYLE_URL && !import.meta.env.VITE_MAP_DARK_TILE_URL
...
function BaseMap({ events = [], selectedId, onSelect, onPick, value, draggable = false, className = '' }) {
  const container = useRef(null); const map = useRef(null); const markers = useRef([]); const picker = useRef(null); const pickHandler = useRef(onPick); const dark = useDarkMode(); const initialDark = useRef(dark); const appliedDark = useRef(dark)
  pickHandler.current = onPick
  useEffect(() => {
    if (!container.current || map.current) return
    map.current = new maplibregl.Map({ container: container.current, style: mapStyle(initialDark.current), center: TANZANIA_CENTER, zoom: 5.1, maxZoom: 19, maxBounds: [[27, -14], [43, 1]], attributionControl: true })
  ...
  useEffect(() => {
    ...
    if (!instance || dark === appliedDark.current) return
    const applyStyle = () => { instance.setStyle(mapStyle(dark)); appliedDark.current = dark }
    if (instance.loaded()) applyStyle(); else instance.once('load', applyStyle)
    return () => instance.off('load', applyStyle)
  }, [dark])
  useEffect(() => {
  ...
  return <div className={`event-map-canvas ${dark ? 'map-dark' : 'map-light'} ${useFilteredDarkTiles(dark) ? 'map-dark-filtered' : ''} ${className}`}><div ref={container}/><button className="map-reset" onClick={reset} type="button" aria-label="Show all Tanzania"><Crosshair/></button></div>
}

export function EventMapDirectory({ events, mode = 'public', loading = false, error, onManage, onToggle, onDelete }) {
  const [search, setSearch] = useState(''); const [selectedId, setSelectedId] = useState(null); const [mobileView, setMobileView] = useState('list'); const [menuId, setMenuId] = useState(null); const refs = useRef({})
  useEffect(() => { const close = () => setMenuId(null); window.addEventListener('click', close); return () => window.removeEventListener('click', close) }, [])
  ...
  return <section className={`event-map-directory mobile-${mobileView}`}>
    <div className="event-map-list"><header><div><h1>{mode === 'admin' ? 'All Events' : mode === 'booking' ? 'Book an event' : 'Events across Tanzania'}</h1><p>{filtered.length} event{filtered.length === 1 ? '' : 's'}</p></div>{mode === 'admin' && <Button asChild><Link to="/admin/events/new"><Plus/>Add an event</Link></Button>}</header><div className="event-map-search"><Search/><Input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search events..." aria-label="Search events"/></div>
      <div className="event-map-scroll">{loading ? <div className="event-map-state">Loading events…</div> : error ? <Alert>{messageOf(error)}</Alert> : filtered.map(event => { const id = event.id || event._id; return <article ref={node => { refs.current[id] = node }} key={id} className={String(selectedId) === String(id) ? 'selected' : ''} onClick={() => select(event)}><div className="event-card-title"><i><CalendarDays/></i><div><h2>{event.name}</h2><p><MapPin/>{event.locationLabel || event.address || event.venue || 'Location not set'}</p></div><div className="event-card-status"><Badge tone={event.status === 'published' || event.status === 'active' ? 'green' : 'orange'}>{event.salesPaused ? 'Disabled' : event.status}</Badge>{mode === 'admin' && <div className="event-action-menu"><button type="button" className="event-ellipsis" aria-label={`Actions for ${event.name}`} onClick={click => { click.stopPropagation(); setMenuId(menuId === id ? null : id) }}><MoreHorizontal/></button>{menuId === id && <div onClick={click => click.stopPropagation()}><button onClick={() => { onManage(event); setMenuId(null) }}><Pencil/>Edit event</button><button onClick={() => { onToggle(event); setMenuId(null) }}>{event.salesPaused ? <PlayCircle/> : <PauseCircle/>}{event.salesPaused ? 'Enable event' : 'Disable event'}</button><button className="danger" onClick={() => { onDelete(event); setMenuId(null) }}><Trash2/>Delete event</button></div>}</div>}</div></div><div className="event-card-dates"><span>Starts <strong>{date(event.startsAt)}</strong></span><span>Ends <strong>{date(event.endsAt)}</strong></span></div>{!hasLocation(event) && <small className="missing-location">Location needs to be set</small>}{action(event)}</article> })}{!loading && !error && !filtered.length && <div className="event-map-state">No matching events found.</div>}</div>
