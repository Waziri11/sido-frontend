...
export function EventMapDirectory({ events, mode = 'public', loading = false, error, onManage }) {
  ...
  return <section className={`event-map-directory mobile-${mobileView}`}>
    ...
    </aside><BaseMap events={filtered} selectedId={selectedId} onSelect={select}/><div className="event-mobile-toggle"><button className={mobileView === 'list' ? 'active' : ''} onClick={() => setMobileView('list')}><List/>List</button><button className={mobileView === 'map' ? 'active' : ''} onClick={() => setMobileView('map')}><MapIcon/>Map</button></div>
  </section>
}
