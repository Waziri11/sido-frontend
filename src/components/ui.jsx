...
export const Field = ({ label, error, children }) => ['businessLicenseDocument', 'tinDocument'].includes(children?.props?.name) ? null : <div><Label>{children?.props?.name === 'physicalAddress' ? 'Physical address (optional)' : label}</Label>{children}{error && <p className="mt-1 text-xs text-red-600" role="alert">{error}</p>}</div>
