...
import { ArrowLeft, ArrowRight, BadgeCheck, Banknote, Building2, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, GraduationCap, LayoutDashboard, Lightbulb, Loader2, LogOut, Mail, Map, Pencil, Search, Settings, ShieldCheck, Store, TicketCheck, Trash2, User, UserRound, Users, Wrench, X } from 'lucide-react'
...
import { AdminBoothControl, AdminCreateEvent, AdminEvents, AdminRevenue, BookingDetail, BookingEvents, MyBookings, PublicEvent, PublicEvents } from './Booking'
...
function Login() {
  ...
  if(session?.user) return <Navigate to={session.user.role==='admin'?'/admin':'/portal'} replace/>
  const submit=async values=>{setBusy(true);setError('');try{const {data}=await api.post('/auth/login',values);if(data.data.requiresOtp){setOtp({email:data.data.email,purpose:data.data.purpose});if(data.data.developmentCode)setError(`Development verification code: ${data.data.developmentCode}`)}else{setSession(data.data);navigate(data.data.user.role==='admin'?'/admin':'/portal')}}catch(e){setError(messageOf(e))}finally{setBusy(false)}}
...
function Protected({role,children}){const {session,loading}=useApp();if(loading)return <PageLoader/>;if(!session?.user)return <Navigate to="/login" replace/>;if(role&&session.user.role!==role)return <Navigate to="/forbidden" replace/>;return children}
...
function PortalShell({admin=false,children}){const {t,logout}=useApp();const navigate=useNavigate();const [open,setOpen]=useState(false);const [collapsed,setCollapsed]=useState(()=>localStorage.getItem('sido_sidebar_collapsed')==='true');const nav=admin?adminNav:entrepreneurNav;const toggleSidebar=()=>setCollapsed(value=>{const next=!value;localStorage.setItem('sido_sidebar_collapsed',String(next));return next});const signout=async()=>{await logout();navigate('/')};return <div className={`portal${collapsed?' sidebar-collapsed':''}`}><aside className={open?'open':''}><div className="sidebar-brand"><button className="mobile-sidebar-close" onClick={()=>setOpen(false)} aria-label="Close navigation"><X/></button></div><div className="panel-label">{admin?t.portal.admin:t.portal.entrepreneur}</div><nav>{nav.map(([to,Icon,label])=><NavLink key={to} to={to} title={collapsed?label:undefined} end={to===(admin?'/admin':'/portal')} onClick={()=>setOpen(false)}><Icon size={19}/><span>{label}</span></NavLink>)}</nav><button className="sidebar-logout" title={collapsed?t.portal.logout:undefined} onClick={signout}><LogOut size={19}/><span>{t.portal.logout}</span></button></aside><div className="portal-main"><SiteHeader portal onMenu={()=>setOpen(true)} onSidebarToggle={toggleSidebar} sidebarCollapsed={collapsed}/><main>{children}</main></div></div>}
...
function App(){return <ErrorBoundary><Routes><Route path="/" element={<Landing/>}/><Route path="/events" element={<PublicEvents/>}/><Route path="/events/:slug" element={<PublicEvent/>}/><Route path="/login" element={<Login/>}/><Route path="/register" element={<Register/>}/><Route path="/verify-email" element={<VerifyEmail/>}/><Route path="/forgot-password" element={<ForgotPassword/>}/><Route path="/reset-password" element={<ResetPassword/>}/><Route path="/portal" element={<Protected role="entrepreneur"><EntrepreneurDashboard/></Protected>}/><Route path="/portal/events" element={<Protected role="entrepreneur"><BookingEvents/></Protected>}/><Route path="/portal/events/:id" element={<Protected role="entrepreneur"><BookingDetail/></Protected>}/><Route path="/portal/bookings" element={<Protected role="entrepreneur"><MyBookings/></Protected>}/><Route path="/portal/profile" element={<Protected role="entrepreneur"><Profile/></Protected>}/><Route path="/portal/settings" element={<Protected role="entrepreneur"><SettingsPage/></Protected>}/><Route path="/admin" element={<Protected role="admin"><AdminDashboard/></Protected>}/><Route path="/admin/events" element={<Protected role="admin"><AdminEvents/></Protected>}/><Route path="/admin/events/new" element={<Protected role="admin"><AdminCreateEvent/></Protected>}/><Route path="/admin/events/:id/booths" element={<Protected role="admin"><AdminBoothControl/></Protected>}/><Route path="/admin/revenue" element={<Protected role="admin"><AdminRevenue/></Protected>}/><Route path="/admin/bookings" element={<Navigate to="/admin/revenue" replace/>}/><Route path="/admin/entrepreneurs" element={<Protected role="admin"><AdminEntrepreneurs/></Protected>}/><Route path="/admin/settings" element={<Protected role="admin"><SettingsPage admin/></Protected>}/><Route path="/forbidden" element={<StatusPage forbidden/>}/><Route path="*" element={<StatusPage/>}/></Routes></ErrorBoundary>}
export default App
