...
function PortalShell({admin=false,children}){const {t,logout}=useApp();const navigate=useNavigate();const [open,setOpen]=useState(false);const nav=admin?adminNav:entrepreneurNav;const signout=async()=>{await logout();navigate('/')};return <div className="portal"><aside className={open?'open':''}><div className="sidebar-brand"><Brand compact/><button onClick={()=>setOpen(false)}><X/></button></div><div className="panel-label">{admin?t.portal.admin:t.portal.entrepreneur}</div><nav>{nav.map(([to,Icon,label])=><NavLink key={to} to={to} end={to===(admin?'/admin':'/portal')} onClick={()=>setOpen(false)}><Icon size={19}/>{label}</NavLink>)}</nav><button className="sidebar-logout" onClick={signout}><LogOut size={19}/>{t.portal.logout}</button></aside><div className="portal-main"><SiteHeader portal onMenu={()=>setOpen(true)}/><main>{children}</main></div></div>}
