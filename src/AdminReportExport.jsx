...
import { Alert, Button, Dialog, DialogContent, DialogTitle, Field, Input as UiInput } from './components/ui'
...
const Input = props => props.name === 'region' ? <select {...props}><option value="">All regions</option>{regions.map(region => <option key={region} value={region}>{region}</option>)}</select> : <UiInput {...props}/>

export default function AdminReportExport({ type, eventId, events = [] }) {
  const { language } = useApp(); const config = configs[type]; const [open, setOpen] = useState(false); const [selected, setSelected] = useState(config.options.map(x => x[0])); const [values, setValues] = useState({}); const [busy, setBusy] = useState(false); const [error, setError] = useState(''); const [availableEvents, setAvailableEvents] = useState(events); const [target, setTarget] = useState(null)
  useEffect(() => {
    const selector = type === 'events' ? '.portal-main>main .event-map-list>header' : '.portal-main>main .page-head'
    const locate = () => { const element = document.querySelector(selector); if (element) setTarget(element) }
